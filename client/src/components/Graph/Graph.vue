<template>
  <svg :width="width" :height="height">
    <g
      ...
      @mousedown.capture="$emit('graph-mouse-down', $event)"
      @contextmenu="$emit('graph-context-menu', $event)"
      cursor="move"
    >
    ...
    <g ref="zoomable">
      <g>
        <line
          ...
          @click.right.prevent="$emit('link-right-click', link)"
          stroke="black"
          stroke-width="2"
        ></line>
      </g>
      <g>
        <!-- <NodeView
          ...
          :cursor="cursors.node"
          @mousedown="$emit('node-mouse-down', $event, node)"
          @nodeMouseUp="$emit('node-mouse-up', $event, node)"
          @dblclick="$emit('node-dbl-click', $event, node)"
          @click.left="$emit('node-left-click', $event, node)"
          @click.right="$emit('node-right-click', $event, node)"
        ></NodeView> -->
        <NodeTemplate
          v-for="node in nodesArr"
          :key="node.id"
        ...
        >
          <template v-slot="data">
            <slot name="NodeTemplate" :node="data"></slot>
          </template>
        </NodeTemplate>
      </g>
    </g>
...
import NodeTemplate from '@/components/Graph/templates/NodeTemplate.vue';
...
class Props {
  nodes = prop<Node[]>({ default: [] });
  links = prop<Link[]>({ default: [] });
  width = prop<number>({ default: 800 });
  height = prop<number>({ default: 600 });
}

@Options({
  components: { NodeTemplate, Selection },
  emits: [
    'graphClick', 'graph-mouse-down', 'graph-mouse-up', 'graph-context-menu',
    'link-click', 'link-right-click',
    'node-mouse-down', 'node-mouse-up', 'node-dbl-click', 'node-left-click', 'node-right-click'
  ]
})
export default class Graph extends Vue.with(Props) implements IGraph {
  $refs!: {
  ...
  protected selectionEnabled: boolean = false;
  ...
  protected onZoomEnd(event: MouseEvent): void {
    ...
    this.$emit('graph-mouse-up', event);
  }
}
</script>
